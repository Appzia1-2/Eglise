...
import { LuSearch, LuPrinter, LuPhone } from "react-icons/lu";
    ...
    <Box bg="white" minH="100vh" display="flex" flexDirection="column">
      ...
      <Container maxW="container.md" flex="1" py={6}>
        ...
        <Box
          ...
          className="no-print"
        >
          <SimpleGrid columns={{ base: 2, md: 5 }} gap={2.5} alignItems="center">
            <Input
              ...
              <Table.Body>
                {members.map((m, index) => (
                  <Table.Row key={m.id} bg={index % 2 === 0 ? "gray.50" : "white"}>
                    ...
                    <Table.Cell fontSize="xs" color="gray.600">
                      <HStack spacing={1}>
                        <Icon as={LuPhone} fontSize="10px" />
                        <Text>{m.mobile_no || "—"}</Text>
                      </HStack>
                    </Table.Cell>
                    <Table.Cell fontSize="xs" color="gray.600">
                      {m.phone_no || "—"}
